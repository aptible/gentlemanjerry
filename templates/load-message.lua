...
-- timestamp for the message, so we'll start by extracting this.  Unfortunately
-- we're not guaranteed that those fields will be here. In particular, app may
-- be missing if JoeCool didn't send it or if Gentlemanjerry failed to parse
-- it. When that happens, we'll just route this message to a junk bucket. This
-- is unlikely to be useful for users, but may let us troubleshoot errors more
-- easily.

-- We used to only support app logs, but now we support databases. To avoid
-- breaking things (mainly, to ease the transition in Dumptruck), we continue
-- routing app keys to their old unprefixed name.

local layer = message.layer or "JUNK"
local messageRouting

if layer == "database" then
	messageRouting = layer .. ":" .. (message.database or "JUNK")
else
	messageRouting = message.app
end

messageRouting = messageRouting or "JUNK"
...
local publishRet = redis.call("PUBLISH", "stream-" .. messageRouting, messageJson)
...
local bufferBucketName = "buffer-bucket-" .. messageRouting .. "-" .. bufferBucketIndex
...
local bufferMapName = "buffer-map-" .. messageRouting
